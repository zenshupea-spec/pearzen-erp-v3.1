...
import { clearProvisionFlashCookie, provisionShalomPortalAccess } from './actions';

export interface ShalomStaffRow {
  epf_number: string;
  full_name: string;
  site: string;
}
...
function buildInitialOtp(initialOtp: GeneratedOTP | null | undefined): GeneratedOTP | null {
  ...
  if (initialOtp.expiresAt <= Date.now()) return null;
  return initialOtp;
}

export default function ShalomPortalClient({
  staff,
  initialOtp = null,
}: {
  staff: ShalomStaffRow[];
      ...
      initialOtp
        ? {
            ...
            expiresAt: initialOtp.otpExpiresAt
              ...
              : Date.now() + SHALOM_PORTAL_OTP_LIFETIME_MS,
          }
        : null,
    ),
  );
  ...
  const handleGenerateOTP = (epf: string) => {
    ...
    startTransition(async () => {
      const result = await provisionShalomPortalAccess(epf);
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm font-semibold leading-relaxed text-slate-600">
        Generate a one-time password for staff assigned to Shalom properties on the MD portfolio
        calendar. They sign in at{' '}
        <span className="font-mono text-xs">/login/shalom-front</span> with EPF + this OTP on first
        login to set a 6-digit PIN, or again if they forgot their PIN. Each code expires after five
        ...
        <div className="space-y-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
          ...
          <div className="rounded-xl border border-emerald-200 bg-white px-4 py-3">
            <div className="flex items-center gap-3">
              <span className="flex-1 font-mono text-3xl font-black tracking-[0.25em] text-emerald-800">
                {generatedOTP.otp}
              </span>
              <button
                type="button"
                onClick={copyOTP}
                className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-300 bg-emerald-100 px-3 py-2 text-xs font-bold text-emerald-900 transition-all active:scale-95"
              >
            ...
            <PortalOtpCountdown
              ...
              lifetimeMs={SHALOM_PORTAL_OTP_LIFETIME_MS}
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center gap-3 border-b border-slate-100 bg-slate-50/80 px-6 py-4">
          ...
          <h2 className="text-sm font-black uppercase tracking-widest text-slate-800">
            Shalom caretakers ({staff.length})
          </h2>
        </div>
        ...
        {staff.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 py-16 text-slate-500">
            ...
            <p className="text-sm font-bold">No caretakers assigned yet</p>
            <p className="max-w-sm px-4 text-center text-xs font-semibold text-slate-400">
              MD must assign an EPF to a property on{' '}
              <span className="font-mono">/executive/shalom</span> before HR can provision front-office
              access here.
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    disabled={isPending}
                    className="inline-flex shrink-0 items-center gap-2 rounded-lg bg-[color:var(--cvs-accent)] px-4 py-2.5 text-xs font-black uppercase tracking-wider text-white shadow-sm transition-all hover:bg-[color:var(--cvs-accent-hover)] active:scale-95 disabled:opacity-50"
                  >
