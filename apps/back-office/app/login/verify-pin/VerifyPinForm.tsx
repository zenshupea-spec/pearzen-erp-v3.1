...
import { signOutPortalSessionAction, verifyHeadOfficePinAction } from './actions';
...
export default function VerifyPinForm({
  ...
  const otpSetupHint = headOfficeOtpExpiryHint(portalRole);
  ...
  const handleSubmit = useCallback(
    (event: React.FormEvent) => {
      ...
      if (!needsSetup && code.length < passwordMinLength) {
        setErrorMsg(`Enter your ${passwordMinLength}+ character password.`);
        return;
      }
      ...
      startTransition(async () => {
        ...
        const result = await verifyHeadOfficePinAction(code, lat, lng);
        if (result?.error) setErrorMsg(result.error);
      });
    },
    [code, geofenceRequired, needsSetup, passwordMinLength],
  );

  const handleSignOut = useCallback(() => {
    startTransition(async () => {
      await signOutPortalSessionAction(signInPath);
    });
  }, [signInPath]);
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                tabIndex={-1}
                aria-readonly
                className="w-full cursor-default rounded-xl border-2 border-slate-200 bg-slate-100 px-4 py-4 text-sm font-semibold text-slate-600 shadow-inner"
              />
            </div>
            ...
            <div>
              <label className="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-600">
                {needsSetup ? 'One-time password (OTP)' : 'Portal password'}
              </label>
              <p className="mb-2 text-center text-xs text-slate-500">
                {needsSetup
                  ? `Use the OTP from HR or MD. ${otpSetupHint} Then choose your permanent password.`
                  : passwordHint}
              </p>
          ...
          <div className="space-y-2 text-center">
            ...
            <button
              ...
              disabled={isPending}
              className="text-xs font-semibold text-slate-500 underline-offset-2 transition-colors hover:text-slate-800 hover:underline disabled:opacity-50"
            >
