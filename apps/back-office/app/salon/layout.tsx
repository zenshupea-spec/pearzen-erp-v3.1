...
import SalonSubnav from './components/SalonSubnav';
import { createSupabaseServerClient } from '../../../../packages/supabase/server';
import { fetchBackOfficeUserProfile } from '../../lib/hr-portal-access-server';
import { loginPathForStaffPortal } from '../../lib/portal-isolation';
import {
  assertSalonVerticalAccessForSession,
  canAccessSalonDesk,
...
export default async function SalonLayout({ children }: { children: ReactNode }) {
  ...
  if (!canAccessSalonDesk(profile.role) && !profile.rbacGated) {
    redirect(`${loginPathForStaffPortal('hq')}?error=salon_denied`);
  }

  const access = await assertSalonVerticalAccessForSession(profile.role);
  if ('error' in access) {
    ...
    redirect(`${hubPath}?error=salon_vertical_inactive`);
  }

  return (
    <main className="min-h-screen bg-gradient-to-b from-rose-50 via-white to-slate-50 px-4 py-8 pb-24 font-sans text-slate-900 md:px-8">
      <StaffPortalChrome />
