...
import { setShalomFrontPinAction } from './actions';

function normalizePinInput(value: string): string {
  return value.replace(/\D/g, '').slice(0, SHALOM_FRONT_PIN_LENGTH);
}

export default function ShalomFrontSetPinPage() {
  ...
  const handleProceedToConfirm = () => {
    if (pin.length !== SHALOM_FRONT_PIN_LENGTH) {
      setErrorMsg(`Enter all ${SHALOM_FRONT_PIN_LENGTH} digits.`);
      return;
    }
    setErrorMsg('');
    setStep('confirm');
  ...
  const handleSetPin = () => {
    if (confirmPin.length !== SHALOM_FRONT_PIN_LENGTH) {
      setErrorMsg(`Confirm all ${SHALOM_FRONT_PIN_LENGTH} digits.`);
      return;
    }
    if (pin !== confirmPin) {
    ...
    startTransition(async () => {
      const result = await setShalomFrontPinAction(pin);
      if (result?.error) {
        setErrorMsg(result.error);
        return;
      }
      window.location.assign('/shalom-front');
    });
  };
      ...
      <div className="mx-auto w-full max-w-sm space-y-8">
        <div className="space-y-3 text-center">
          ...
          <p className="whitespace-pre-line font-mono text-sm leading-relaxed text-slate-500">
            {step === 'enter'
              ? `Choose a ${SHALOM_FRONT_PIN_LENGTH}-digit PIN only you will know.\nYou will use this every time you log in.`
        ...
        <div className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="space-y-3">
            ...
            <input
              ...
              maxLength={SHALOM_FRONT_PIN_LENGTH}
              value={activeValue}
              onChange={(e) => setActiveValue(normalizePinInput(e.target.value))}
              placeholder={`${SHALOM_FRONT_PIN_LENGTH}-digit PIN`}
          ...
          {step === 'enter' ? (
            <button
              ...
              disabled={pin.length !== SHALOM_FRONT_PIN_LENGTH}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={isPending || confirmPin.length !== SHALOM_FRONT_PIN_LENGTH}
