...
export function CafeCheckinButton({ onCheckedIn }: { onCheckedIn?: () => void }) {
  ...
  const refreshContext = useCallback(async () => {
    ...
    if (ctx.checkedOutToday) {
      setUiState('SECURED');
      setSecuredMessage('Shift complete — checked out for today');
      return;
    }
    if (ctx.activeOnShift) {
      setUiState('SECURED');
      setSecuredMessage('Already checked in today');
      return;
    }
    if (ctx.afterClose) {
      setUiState('CLOSED');
    ...
    setUiState(ctx.canCheckIn ? 'IDLE' : 'CLOSED');
  }, []);
  ...
  const beginCheckin = async () => {
    ...
    if (!context.canCheckIn) {
      if (context.afterClose) {
        setUiState('CLOSED');
      } else if (context.checkedInToday) {
  ...
  const captureAndSubmit = async () => {
    ...
    if (result.ok) {
      setSecuredMessage('Checked in — portal unlocked');
      setUiState('SECURED');
      setTimeout(() => {
        onCheckedIn?.();
  ...
  const handleMainButton = () => {
    if (uiState === 'OUT_OF_BOUNDS' || uiState === 'CLOSED') {
      setDistanceAway(null);
      setErrorMessage(null);
      setUiState(context?.withinOpenHours ? 'IDLE' : 'CLOSED');
      return;
    }
    beginCheckin();
  };
  ...
  let buttonText = '';
  let subText = '';
  let disabled = uiState === 'LOADING' || uiState === 'SYNCING';
  ...
  switch (uiState) {
    ...
    case 'CLOSED':
      dynamicStyle =
        'border-slate-300 bg-slate-400 text-white opacity-90 hover:bg-slate-500';
      buttonText = 'Closed';
      subText = context
        ? context.afterClose
          ? `Closed — check out only after ${formatTime(context.cafeOpenEnd)}`
          : `Opens ${formatTime(context.cafeOpenStart)} – ${formatTime(context.cafeOpenEnd)}`
        : 'Outside café hours';
      break;
    case 'SECURED':
      dynamicStyle =
      ...
      buttonText = 'Verified';
      subText = securedMessage || 'On shift';
      disabled = true;
      break;
    case 'IDLE':
    default:
      if (!context?.canCheckIn) {
        dynamicStyle =
          'cursor-not-allowed border-slate-300 bg-slate-400 text-white opacity-90';
        buttonText = 'Closed';
        subText = context?.afterClose
          ? `After ${formatTime(context.cafeOpenEnd)} — no new check-ins`
          : `${formatTime(context?.cafeOpenStart ?? '07:00')} – ${formatTime(context?.cafeOpenEnd ?? '19:00')}`;
        disabled = true;
      } else {
        dynamicStyle =
        ...
        <div className="w-full rounded-2xl border border-slate-200/80 bg-white/90 px-4 py-3 text-center shadow-sm">
          ...
          {context.shiftType === 'MORNING' || context.shiftType === 'EVENING' ? (
            <p className="mt-1 text-[10px] font-bold uppercase tracking-wider text-slate-500">
              {formatCafeShiftWindowLabel(context.shiftType, context.shiftWindows)}
            </p>
          ) : null}
        </div>
      ) : null}
