-- Head Office vault session admin (Active Vault Sessions panel)
...
as $$
  ...
  join public.head_office_portal_auth h
    on lower(trim(h.work_email)) = lower(trim(u.email))
...
revoke all on function public.list_active_head_office_vault_sessions(uuid) from public;
grant execute on function public.list_active_head_office_vault_sessions(uuid) to service_role;
...
revoke all on function public.revoke_head_office_vault_session(uuid, uuid) from public;
grant execute on function public.revoke_head_office_vault_session(uuid, uuid) to service_role;
...
revoke all on function public.revoke_other_head_office_vault_sessions(uuid, uuid) from public;
grant execute on function public.revoke_other_head_office_vault_sessions(uuid, uuid) to service_role;
