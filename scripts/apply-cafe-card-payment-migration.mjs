/**
 * Applies café card payment columns + RPC updates to remote Supabase.
 * Run: npm run db:apply-cafe-card-payment
 */
...
const sqlText = readFileSync(
  join(root, 'packages/supabase/migrations/20260610150000_cafe_order_card_payment.sql'),
  'utf8',
);
...
async function applyViaManagementApi() {
  ...
  console.log('Applied café card payment migration (Management API).');
  return true;
}
...
async function applyViaPostgres() {
  ...
  try {
    ...
    console.log('Applied café card payment migration (direct Postgres).');
    return true;
  } finally {
...
console.error(
  'Could not apply migration.\n' +
    ...
    '  npm run db:apply-cafe-card-payment',
);
process.exit(1);
