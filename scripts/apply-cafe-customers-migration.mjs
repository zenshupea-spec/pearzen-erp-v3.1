/**
 * Café customer registry + phone lookup.
 * Run: npm run db:apply-cafe-customers
 */
...
const sqlText = readFileSync(
  join(root, 'packages/supabase/migrations/20260610170000_cafe_customers.sql'),
  'utf8',
);
...
async function applyViaManagementApi() {
  ...
  console.log('✅ Applied cafe customers migration via Management API');
  return true;
}
...
async function applyViaPg() {
  ...
  try {
    ...
    console.log('✅ Applied cafe customers migration via DATABASE_URL');
    return true;
  } finally {
