/**
 * Employee fixed allowances + FM payroll earnings adjustments.
 * Run: npm run db:apply-employee-allowances
 */
...
const migrationPath = join(
  root,
  'packages/supabase/migrations/20260610270000_employee_fixed_allowances.sql',
);
...
async function applyViaManagementApi() {
  ...
  console.log('✅ Applied employee allowances migration (Management API)');
  return true;
}
...
async function applyViaPostgres(dbUrl) {
  ...
  try {
    ...
    console.log('✅ Applied employee allowances migration (Postgres)');
    return true;
  } finally {
