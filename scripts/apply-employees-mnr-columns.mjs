/**
 * Applies employees MNR PII + vetting columns to remote Supabase.
 * Run: npm run db:apply-mnr-columns
 */
...
const migrationPath = join(
  root,
  'packages/supabase/migrations/20260608190000_employees_mnr_pii_columns.sql',
);
...
async function applyViaManagementApi() {
  ...
  console.log('✅ Applied employees MNR columns (Management API)');
  return true;
}
...
async function applyViaPostgres(dbUrl) {
  ...
  try {
    ...
    console.log('✅ Applied employees MNR columns (Postgres)');
    return true;
  } finally {
