/**
 * Applies fleet telematics tables to remote Supabase.
 * Run: npm run db:apply-fleet-telematics
 */
...
const migrationPath = join(
  root,
  'packages/supabase/migrations/20260609250000_fleet_telematics.sql',
);
...
async function applyViaManagementApi() {
  ...
  console.log('✅ Applied fleet telematics migration (Management API)');
  return true;
}
...
async function applyViaPostgres(dbUrl) {
  ...
  try {
    ...
    console.log('✅ Applied fleet telematics migration (Postgres)');
    return true;
  } finally {
