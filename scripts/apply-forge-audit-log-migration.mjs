/**
 * Apply forge_audit_log migration (FORGE_CVS_ISOLATION S-31).
 * Run: npm run db:apply-forge-audit-log
 */
...
const migrationFile = '20260626140000_forge_audit_log.sql';
...
async function verifyTable() {
  ...
  const res = await fetch(`https://api.supabase.com/v1/projects/${projectRef}/database/query`, {
    ...
    body: JSON.stringify({
      query: `SELECT column_name FROM information_schema.columns
              WHERE table_schema = 'public' AND table_name = 'forge_audit_log'
  ...
  if (cols.length) console.log('Verified columns:', cols.join(', '));
}
