/**
 * Payroll runs + duplicate-safe payslips.
 * Run: npm run db:apply-payroll-runs
 */
...
const sqlText = readFileSync(
  join(root, 'packages/supabase/migrations/20260610190000_payroll_runs_payslips.sql'),
  'utf8',
);
...
async function applyViaManagementApi() {
  ...
  console.log('✅ Applied payroll runs migration via Management API');
  return true;
}
...
async function applyViaPg() {
  ...
  try {
    ...
    console.log('✅ Applied payroll runs migration via DATABASE_URL');
    return true;
  } finally {
