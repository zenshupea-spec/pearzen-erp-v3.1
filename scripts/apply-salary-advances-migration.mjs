/**
 * FM salary_advances payroll-period columns + unique constraint.
 * Run: npm run db:apply-salary-advances
 */
...
const sqlText = readFileSync(
  join(root, 'packages/supabase/migrations/20260610220000_salary_advances_fm.sql'),
  'utf8',
);
...
async function applyViaManagementApi() {
  ...
  console.log('✅ Applied salary_advances FM migration via Management API');
  return true;
}
...
async function applyViaPg() {
  ...
  try {
    ...
    console.log('✅ Applied salary_advances FM migration via DATABASE_URL');
    return true;
  } finally {
