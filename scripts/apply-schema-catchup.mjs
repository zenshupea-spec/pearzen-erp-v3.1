/**
 * Applies the full schema catch-up bundle to remote Supabase.
 * Run: npm run db:apply-schema-catchup
 *
 * Requires one of:
...
async function applyViaManagementApi() {
  ...
  console.log('✅ Applied schema catch-up bundle (Management API)');
  return true;
}
...
async function applyViaPostgres(dbUrl) {
  ...
  try {
    ...
    console.log('✅ Applied schema catch-up bundle (Postgres)');
    return true;
  } finally {
...
console.error(
  'Could not apply migration automatically.\n' +
    ...
    'Then run: npm run db:apply-schema-catchup\n\n' +
