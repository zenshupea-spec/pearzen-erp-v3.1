/**
 * SM portal OTP hash column + otp_expires_at (required for HR induction + SM login).
 * Run: npm run db:apply-sm-portal-otp-hash
 */
...
async function applyViaManagementApi() {
  ...
  console.log('✅ Applied SM portal auth OTP migrations via Management API');
  return true;
}
...
async function applyViaPg() {
  ...
  try {
    ...
    console.log('✅ Applied SM portal auth OTP migrations via DATABASE_URL');
    return true;
  } finally {
...
if (!ok) {
  ...
  console.error('   npm run db:apply-sm-portal-otp-hash');
  process.exit(1);
}
